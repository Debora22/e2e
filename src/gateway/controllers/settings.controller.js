const { controller } = require('jimpex');

/**
 * The controller for the settings resource.
 */
class SettingsController {
  /**
   * @param {AdminAPI2}        adminAPI2         To make requests to the Admin API2.
   * @param {ResponsesBuilder} responsesBuilder  To format the received responses.
   */
  constructor(
    adminAPI2,
    responsesBuilder,
  ) {
    /**
     * The local reference to the `adminAPI2` service.
     *
     * @type {AdminAPI2}
     */
    this.adminAPI2 = adminAPI2;
    /**
     * The local reference to the `responsesBuilder` service.
     *
     * @type {ResponsesBuilder}
     */
    this.responsesBuilder = responsesBuilder;
  }
  /**
   * Use the Admin API2 to save the customer analytics settings.
   *
   * @return {ExpressMiddleware}
   */
  saveAnalyticsSettings() {
    return (req, res, next) => {
      this.adminAPI2.saveAnalyticsSettings(req, req.body)
      .then((response) => this.responsesBuilder.json(res, response))
      .catch((error) => next(error));
    };
  }
}

/**
 * This controller adds the routes for the settings resource.
 *
 * @type {Controller}
 */
const settingsController = controller((app) => {
  const router = app.get('router');

  const ctrl = new SettingsController(
    app.get('adminAPI2'),
    app.get('responsesBuilder'),
  );

  return router
  .all('*', app.get('ensureBearerToken'))
  .put('/analytics', ctrl.saveAnalyticsSettings());
});

module.exports = {
  SettingsController,
  settingsController,
};
